// ==================== ON LOAD ======================
  if (document.readyState === 'complete' || document.readyState !== 'loading' && !document.documentElement.doScroll) {
    ...
    document.addEventListener('DOMContentLoaded', async() => {
      ...
      const northEast = [10.759257733272932, 122.59514534895315];
      const southWest = [10.687196160454304, 122.46682847244875];
      ...
      apiUrl = (mode=="prod" ? "api/ltaodataservice/all/BusStops" : "../data/JeepneyStops.json");
      ...
      if(!busStopsRetrieved) {
        ...
        response = await fetch("../data/JeepneyStops.json", apiHeaders);
        responseObj = await response.json();
      ...
      apiUrl = (mode=="prod" ? "api/ltaodataservice/all/BusServices" : "../data/JeepneyServices.json");
      ...
      if(!busServicesRetrieved) {
        apiHeaders["method"]="GET";
        response = await fetch("../data/JeepneyServices.json", apiHeaders);
        responseObj = await response.json();
      ...
      if(mode=="dev") {
        response = await fetch("../data/JeepneyRoutes.json", apiHeaders);
        responseObj = await response.json();
        if(responseObj.length==0) {
      ...
      function sidebarClick(busSvcNo) {
        ...
        selectedjeepneyServiceRouteObjs=[];
